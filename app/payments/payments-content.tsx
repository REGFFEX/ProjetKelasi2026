...
import { Wallet, Plus, Search, Download, FileText, ArrowLeft, Save, CreditCard, TrendingUp, AlertCircle, CheckCircle } from 'lucide-react';
import { invoices, payments, students, getStudentName, getClassroomName, formatMoney } from '@/lib/mock-data';
...
export function PaymentsContent() {
  ...
  const totalPaid = invoices.reduce((sum, inv) => sum + inv.montantPaye, 0);
  const totalDue = invoices.reduce((sum, inv) => sum + inv.resteAPayer, 0);
  const totalRevenue = payments.reduce((sum, p) => sum + p.montant, 0);

  const handlePayment = (invoice: Invoice) => {
    setSelectedInvoice(invoice);
    setPaymentAmount(String(invoice.resteAPayer));
    setView('payment');
  };
            ...
            <table className="w-full min-w-[700px]">
              ...
              <tbody className="divide-y divide-border">
                {filteredInvoices.map(inv => {
                  const cfg = statusConfig[inv.statut];
                    ...
                    <tr key={inv.id} className="hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-foreground truncate max-w-[180px]">{getStudentName(inv.studentId)}</p>
                            <p className="text-xs text-muted-foreground truncate">{getClassroomName(student?.classroomId || '')}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-foreground capitalize truncate max-w-[140px]">{inv.typeFrais}</td>
                      <td className="px-4 py-3 text-right text-sm text-foreground whitespace-nowrap">{formatMoney(inv.montantTotal)}</td>
                      <td className="px-4 py-3 text-right text-sm text-success font-medium whitespace-nowrap">{formatMoney(inv.montantPaye)}</td>
                      <td className="px-4 py-3 text-right text-sm text-destructive font-medium whitespace-nowrap">{formatMoney(inv.resteAPayer)}</td>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-foreground truncate">{getStudentName(inv.studentId)}</p>
                        <p className="text-xs text-muted-foreground capitalize truncate">{inv.typeFrais} · {getClassroomName(student?.classroomId || '')}</p>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 text-center">
                      <div>
                        ...
                        <p className="text-sm font-medium text-foreground truncate">{formatMoney(inv.montantTotal)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Payé</p>
                        <p className="text-sm font-medium text-success truncate">{formatMoney(inv.montantPaye)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Reste</p>
                        <p className="text-sm font-medium text-destructive truncate">{formatMoney(inv.resteAPayer)}</p>
                      </div>
                    </div>
                    {inv.resteAPayer > 0 && (
  ...
  if (view === 'payment' && selectedInvoice) {
    ...
    const newReste = selectedInvoice.resteAPayer - amount;
    const isPartial = amount < selectedInvoice.resteAPayer && amount > 0;
    const exceedsReste = amount > selectedInvoice.resteAPayer;
        ...
        <Card className="p-4 sm:p-6 rounded-2xl shadow-card hover:shadow-card-hover transition-shadow">
          ...
          <div className="bg-muted/30 rounded-xl p-4 mb-6">
            <div className="flex items-center justify-between mb-3 gap-2">
              <h3 className="font-semibold text-foreground truncate">{getStudentName(selectedInvoice.studentId)}</h3>
              <span className="text-xs text-muted-foreground capitalize truncate">{selectedInvoice.typeFrais}</span>
            </div>
            <div className="grid grid-cols-3 gap-3 sm:gap-4">
              <div>
                <p className="text-xs text-muted-foreground">Montant total</p>
                <p className="text-sm font-bold text-foreground truncate">{formatMoney(selectedInvoice.montantTotal)}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Déjà payé</p>
                <p className="text-sm font-bold text-success truncate">{formatMoney(selectedInvoice.montantPaye)}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Reste à payer</p>
                <p className="text-sm font-bold text-destructive truncate">{formatMoney(selectedInvoice.resteAPayer)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              {amount === selectedInvoice.resteAPayer && amount > 0 && (
            ...
            <div className="bg-muted/30 rounded-xl p-4 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-foreground">{formatMoney(selectedInvoice.resteAPayer)}</span>
              </div>
              <div className="flex justify-between text-sm">
        ...
        <Card className="p-4 sm:p-8 rounded-2xl shadow-card hover:shadow-card-hover transition-shadow">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-6">
            <div className="min-w-0">
              ...
              <p className="text-sm font-medium text-foreground truncate">{getStudentName(selectedInvoice.studentId)}</p>
              <p className="text-xs text-muted-foreground truncate">{student ? getClassroomName(student.classroomId) : ''}</p>
            </div>
            <div>
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm font-medium text-foreground capitalize truncate">{selectedInvoice.typeFrais}</p>
            </div>
            <div>
          ...
          <div className="overflow-x-auto scrollbar-thin mb-6">
            <table className="w-full min-w-[400px]">
              ...
              <tbody className="divide-y divide-border">
                <tr>
                  <td className="px-3 py-2.5 text-sm text-foreground">Frais de {selectedInvoice.typeFrais} — {getStudentName(selectedInvoice.studentId)}</td>
                  <td className="px-3 py-2.5 text-right text-sm font-medium text-foreground whitespace-nowrap">{formatMoney(selectedInvoice.montantTotal)}</td>
                </tr>
                {invoicePayments.map(p => (
                  <tr key={p.id}>
                    <td className="px-3 py-2.5 text-sm text-success">Paiement ({methodLabels[p.modePaiement]}) — {new Date(p.datePaiement).toLocaleDateString('fr-FR')}</td>
              ...
              <tfoot>
                <tr className="border-t-2 border-border bg-primary/5">
                  ...
                  <td className="px-3 py-3 text-right">
                    <span className={cn('text-lg font-bold', selectedInvoice.resteAPayer <= 0 ? 'text-success' : 'text-destructive')}>
                      {formatMoney(selectedInvoice.resteAPayer)}
                    </span>
                  </td>
                </tr>
              </tfoot>
